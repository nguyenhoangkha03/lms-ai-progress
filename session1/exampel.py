...
class Learning_assessment:
    ...
    def _get_session_analytics(self, user_id: str, now):
        where_clause = f""" 
        WHERE lea.studentId = '{user_id}' AND MONTH(lea.date) = '{now.month} ORDER BY lea.date ASC'
        """
...
class RandomForestLearninAttube:
    ...
    def extract_features_lesson(self, data):
        def safe_mean(data_list):
            return round(np.mean(data_list), 2) if data_list else 0.0

        def safe_std(data_list):
            return round(np.std(data_list), 2) if data_list else 0.0
        ...
        days_study = session_analytics.get('days_study', 0)
        ...
        percent_time_study = attitude.get('percent_time_study', 0)
        ...
        pre_data_user = [
            days_study,
            round((days_study / month_now), 2) if month_now > 0 else 0,
            safe_std(days_gap_study),
            safe_mean(days_off),
            safe_std(days_off),
            round(sum(timeSpentInMonth_list), 2),
            safe_mean(timeSpentInMonth_list),
            safe_std(timeSpentInMonth_list),
            safe_mean(duration_list),
            safe_std(duration_list),
            safe_mean(activities_list),
            safe_std(activities_list),
            percent_time_study
        ]
    ...
    def _determine_attitude_id(self, features: np.array) -> int:
        days_study = features[0]
        mean_days_study_rate = features[1]
        std_days_gap = features[2]
        mean_session_gap = features[3]
        total_time = features[5]
        mean_time = features[6]
        percent_time_study = features[12]

        # Hard_work (ID 0)
        if mean_days_study_rate > 0.5 and std_days_gap < 2.5 and percent_time_study > 60:
            return 0

        # Give_up (ID 3)
        if days_study < 4 and total_time < 8000:
            return 3

        # Cramming (ID 4)
        if days_study < 10 and mean_time > 8500:
            return 4

        # Lazy (ID 2)
        if mean_session_gap > 4 and percent_time_study < 45:
            return 2

        # Distraction (ID 1)
        if features[10] > 7 and features[8] > 7000:
            return 1
    ...
    def train(self):
        ...
        learning_assessor = Learning_assessment(self.db)
        where_clause = """
        JOIN user_roles url ON url.user_id = ur.id
        JOIN roles rl ON rl.id = url.role_id
        WHERE rl.name = 'student'
        ...
        for student in students:
            ...
            analytics_data = learning_assessor.learning_analytics_data(student_id)

            features = self.extract_features_lesson(analytics_data)
            if np.isnan(features).any() or np.isinf(features).any():
                continue

            attitude_id = self._determine_attitude_id(features)
            
            # feature_names = [
            #     'days_study', 'mean_days_study_rate', 'std_days_gap', 'mean_session_gap',
            #     'std_session_gap', 'total_time', 'mean_time', 'std_time', 'mean_duration',
            #     'std_duration', 'mean_activities', 'std_activities', 'percent_time_study'
            # ]
            # tqdm.write(f"\n----------------------------------------")
            # tqdm.write(f"Student ID: {student_id}")
            # tqdm.write("  Features:")
            # for i, name in enumerate(feature_names):
            #     tqdm.write(f"    - {name}: {features[i]}")
            # tqdm.write(f"  --> Attitude ID: {attitude_id} ({self.Learning_attitude.get(attitude_id, 'Unknown')})")
            # tqdm.write(f"----------------------------------------")
            # ===============================================

            X_train.append(features)
            y_train.append(attitude_id)


        if not X_train:
            print("Không có đủ dữ liệu để huấn luyện model.")
            return None
        ...
        if min_class_count < 2:
            print(f"⚠️  Lớp nhỏ nhất chỉ có {min_class_count} thành viên. Không thể thực hiện GridSearchCV.")
            X_train_scaled = self.scaler.fit_transform(X_train)
            self.model.fit(X_train_scaled, y_train)
            self.is_trained = True
            print("✅ Huấn luyện cơ bản hoàn tất.")
            return self.model
        ...
        n_splits = min(5, min(Counter(y_train_split).values()))
        # print(f"ℹ️ Tự động điều chỉnh GridSearchCV cv={n_splits} vì lớp nhỏ nhất trong tập train có {min(Counter(y_train_split).values())} thành viên.")
        ...
        print("🔍 Training Attitude Model...")
        with tqdm_joblib(tqdm(total=len(ParameterGrid(param_grid)) * n_splits, disable= True    )):
            grid_search.fit(X_train_scaled, y_train_split)
        ...
        print(f"   Accuracy: {accuracy_score(y_test_split, y_pred):.2%}")
        target_names = [self.Learning_attitude[i] for i in sorted(self.Learning_attitude.keys())]
        print(classification_report(
            y_test_split,
            y_pred,
            labels=list(self.Learning_attitude.keys()),
            target_names=target_names,
            digits=3,
            zero_division=0
        ))
    ...
    def predict_attitude(self, data, return_proba=False):
        if not self.is_trained:
            ...
            if not self.is_trained:
                raise RuntimeError(
                    "Không thể huấn luyện model do không đủ dữ liệu hoặc dữ liệu không đa dạng. "
                    "Không thể thực hiện dự đoán. Vui lòng kiểm tra dữ liệu đầu vào hoặc các quy tắc "
                    "trong hàm _determine_attitude_id()."
                )
        ...
        total_day_off = sum(sessions.get('day_off'), 0)
        duration_session = []
        activities_session = []
        for s in session:
            duration_session.append(s.get('duration'))
            activities_session.append(s.get('activitiesCount'))
        total_duration = sum(duration_session, 0) / 3600
        total_activitie = sum(activities_session, 0)
        ...
        result = {
            ...
            'reason': f"Bạn đã nghỉ {total_day_off} ngày, tổng thời gian online là {total_duration} giờ và tổng số thao tác là {total_activitie}"
        }

        if return_proba:
            result['probabilities'] = {
                attitude_name: float(prob_dict.get(class_id, 0.0))
                for class_id, attitude_name in self.Learning_attitude.items()
            }
